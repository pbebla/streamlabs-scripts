...
def Execute(data):
	...
	if data.IsChatMessage() and data.GetParam(0).lower() == settings["command"] and Parent.HasPermission(data.User, settings["permission"], ""):
		...
		if data.IsFromDiscord():
			Parent.SendDiscordMessage(tempResponseString)
	return
...
def ReloadSettings(jsonData):
	...
	jsonData = json.loads(Parent.GetRequest("https://api.frankerfacez.com/v1/set/240", {}))
	if jsonData["status"] == 200:
		for emote in json.loads(jsonData['set'])['emoticons']:
			twitchEmotes.add(emote['name'])
